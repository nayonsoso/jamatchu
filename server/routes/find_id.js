...
router.post("/", function(req, res) {
    let name = req.body.i_name;
    let question = req.body.i_question;
    let asw = req.body.i_asw;
    var sql = "SELECT * FROM user WHERE user_realname = ? AND user_q=? AND user_a=?;"
    conn.query(sql,[name, question, asw],function(err, rs){
        if(Object.keys(rs).length===0){
            ...
            let _name = rs[0].user_realname;
            let _a = rs[0].user_a;
            let _id = rs[0].user_id;
            res.send({name:_name, q:_q,a:_a,check:_check, id:_id});
        }
    })
})
